				...
				<div class="page">
					<div class="content">
							<div class="row">
								...
								<div class="col-md-6">
									<figure class="movie-poster"><img src="http://image.tmdb.org/t/p/original/${serie.posterPath}" alt="${serie.name}"></img></figure>
								</div>
								<div class="col-md-6">
									...
									<ul class="movie-meta">
										<li><strong>Rating:</strong> 
											<div class="star-rating" title="Rated ${serie.voteAverage}.00 out of 5"><span style="width:80%"><strong class="rating">${serie.voteAverage}.00</strong> out of 5</span></div>
										</li>
										<li>
											<strong>Episode runtime (min): </strong>
											<c:forEach items="${serie.episodeRunTime}" var="runtime" varStatus="status">
												${runtime} ${not status.last ? ',':''}
											</c:forEach>
										</li>
										<li><strong>First Episode:</strong> ${serie.firstAirDate}</li>
										<li>
											<strong>Category:</strong>
											<c:forEach items="${serie.genres}" var="genero" varStatus="status">
									...
									<ul class="starring">
										...
										<li><strong>Creators:</strong>
											<ul class="starring">
												<c:forEach items="${serie.createdBy}" var="creador">
													<li> ${creador.name}</li>
												</c:forEach>
											</ul>
										</li>
										...
										<li>
										<div class="row">
											<c:forEach items="${serie.topCast}" var="actor">
												<div class="col-sm-2">
										...
										<li><strong>Number of Seasons: </strong>${serie.numberOfSeasons}</li>
										<!--  										
										<li>
											<c:forEach items="${serie.seasons}" var="season">
												<div class="col-sm-2">
													<div class="latest-movie">
														<figure>
															<img alt="${season.name}" src="http://image.tmdb.org/t/p/original/${season.posterPath}"/>
															<figcaption>${season.name}</figcaption>
														</figure>
													</div>
												</div>
											</c:forEach>
						...
						<div class="row">
							<h3>Available in:</h3>
							<div class="filters">
								<form method="POST" action="${serie.id}" id="filtro">
									<select name="pais" id="pais" form="filtro">
										<option selected>-- Choose Country --</option>
										<c:forEach items="${countries}" var="country">
											<option value="${country.iso_639_1}">${country.name}</option>
										</c:forEach>
									</select>
